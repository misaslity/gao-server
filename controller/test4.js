...
const test4 = async (req, res) => {
  ...
  async function fetchAndInsertDistricts() {
    for (const province of provinces) {
      ...
      try {
        ...
        //   const query = 'INSERT INTO districts SET ?';
        //   connection.query(query, district, (err, result) => {
        //     if (err) throw err;
        //     console.log('Inserted district:', result.insertId);
        //   });
          await fetchAndInsertWards(district.district_id);
        }
      } catch (error) {
        console.error(`Error fetching districts for province ${province.province_id}:`, error);
      }
    }
  }
  fetchAndInsertDistricts()
  return res.json({ok: true})
};

async function fetchAndInsertWards(districtId) {
    ...
    try {
      ...
      for (const ward of wards) {
        const query = 'INSERT INTO wards SET ?';
        connection.query(query, ward, (err, result) => {
          if (err) throw err;
          console.log('Inserted ward:', result.insertId);
        });
      }
    } catch (error) {
