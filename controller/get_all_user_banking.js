...
const get_all_user_banking = async (req, res) => {
  try {
    ...
    const [countRows] = await connection.query(
      "SELECT COUNT(*) AS total FROM user WHERE username LIKE ? AND role != 'admin' AND account_number IS NOT NULL",
      [`%${searchKeyword}%`]
    );
    ...
    const [rows] = await connection.query(
      `SELECT * FROM user WHERE username LIKE ? AND role != 'admin' AND account_number IS NOT NULL ORDER BY id DESC LIMIT ?, ?`,
...
module.exports = get_all_user_banking;
