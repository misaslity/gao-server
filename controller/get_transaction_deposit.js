...
const getTransactionDeposit = async (req, res) => {
  try {
    ...
    const [rows] = await connection.query(
      `SELECT *, deposit_history.id AS id 
       FROM deposit_history 
       LEFT JOIN user ON user.id = deposit_history.user_id 
       WHERE UNIX_TIMESTAMP(STR_TO_DATE(SUBSTRING(deposit_history.time_created, 5, 20), '%b %d %Y %H:%i:%s')) BETWEEN ? AND ?
       ORDER BY deposit_history.id DESC 
       LIMIT ? OFFSET ?`,
